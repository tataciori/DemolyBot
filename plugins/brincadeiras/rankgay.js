...
export default {
    command: ['rankgay', 'topgay'],
    description: 'Top 5 mais gays do grupo.',
    async run(sock, msg, args) {
        ...
        try {
            ...
            const lines = selected.map((p, i) => `#${i + 1} 🏳️‍🌈 @${p.id.split('@')[0]}`);
            const mentions = selected.map(p => p.id);
            
            const texto = `🌈 *RANK - MAIS GAYS DO GRUPO* 🌈\n\n${lines.join('\n')}`;
            await sock.sendMessage(chatId, { text: texto, mentions });
        } catch (e) {
            console.error('Erro no rankgay:', e);
